      ...
      <div className='max-w-[1200px] mx-auto'>
        ...
        <div className='md:text-xl sm:text-[25px] text-white text-center grid lg:gap-5 md:gap-3 lg:grid-cols-5 sm:grid-cols-3'>
          ...
          {isLoggedIn && isAdmin === 'T' && <SkillCard>
            <Button className='h-full w-full rounded-xl' icon={<PlusOutlined />} type='dashed'>Add Skill</Button>
            </SkillCard>}
        </div>
      </div>
    </div>
...
function SkillCard({id, source, label, children}) {
  ...
  return (
    ...
    {isLoggedIn && isAdmin=== 'T' && mouseEnter && id && <Button className='absolute -top-2 -right-2 w-2 text-xl rounded-full' ><CloseCircleOutlined/></Button>}
    </div>
    </>
  )
}